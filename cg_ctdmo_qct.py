...
# ---------------------------------------------------------------------------- #
#
# Automates the QCT procedure for OOI CTDMO instruments (SBE 37IM).
#
...
# "3305-00101-00000.docx" and the CTDMO inventory file "ctdmo_inv.csv" are in
# the same directory as this script file. The files generated by the script will
# be automatically saved in the same directory.
#
# Allen Smith
# gsmith@whoi.edu
# ---------------
...
def set_formnumber(formnumber):
    ...
    if not formnumber:
        ...
        while not input_validated:
            formnumber = raw_input("Enter the five-digit form number (ex: 00123): ")
...
def get_remote_id(ser, capfile):
    ...
    while True:
        ...
        if not remote_id:
            ...
            if raw_input("Would you like to try again? [y]/n ") == "n":
                return None
        else:
...
def ctdmo_qct_test(port, username, formnumber):
    ...
    try:
        ...
        while True:
            seriesletter = raw_input("Enter the instrument Series (G, H, Q or R): ").upper()
    ...
    ser_coms(ser, capfile, '#%sgetcc' % remote_id)
    ...
    while not this_step_complete:
        ...
        if not 0 <= (rollback_instr - rollback_target).seconds <= 15:
            ...
            response = raw_input("Does this bother you, %s? [y]/n "
                                 % username.split()[0])
            if response != "n":
...
while not time_to_quit:
    ...
    if selection == '1':
        ...
        while True:
            ...
            again = raw_input("Would you like to test another instrument? y/[n] ")
            if again != "y":
                formnumber = None
                break
            raw_input("Type ENTER to begin the next test...")
    elif selection == '2':
        port = select_port()
